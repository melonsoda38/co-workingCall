...
/** 最終休憩で続行ボタンの上に表示する続行案内テキスト (Timer 画像の外・太字)。 */
 ...
 * - content: 最終休憩フェーズのみ、Timer 画像の外 (Embed の上) に太字で "**続ける場合:**" を
 *   表示し、Embed 下に「続行」ボタンを付ける。それ以外のフェーズは空文字。
 *
 * 画像は分刻み更新前提 (TimerEmbedUpdater の間隔 60 秒)。
 *
 * content / components は **常に明示的に返す** (最終休憩=続行案内+ボタン / それ以外=空)。
 * これによりフェーズをまたいで edit された場合でも付け外しが確実に反映される。
 */
export function buildTimerEmbedContent(
  ...
  return {
    content: isFinalBreak ? `**${CONTINUE_PROMPT_TEXT}**` : '',
    embeds: [embed],
    files: [attachment],
