...
どちらのボタンも `/pomo` と同じ許可ロール保持者のみが実行できる。Discord はボタンを
ロール別に非表示にできないため、ボタンは全員に見えるが、押下時にロール判定して
権限がなければ ephemeral で弾く (詳細は commands-spec.md §Embedボタン)。
...
#### 更新タイミング
分刻み表示なので更新間隔は 60 秒。「次の分境界を少し過ぎた所 (安全マージン 50ms)」で
発火させる自己補正 setTimeout チェイン (毎回 getSnapshot() で再計算)。表示は
ceil(残り/60000) で、残りが分境界を下回った瞬間に値が 1 減るため、境界の手前で撃つと
旧値 (1 分多い値) を描いて表示が約 1 分遅れる。よって境界の内側で撃つ。setInterval は
再アームごとのドリフトが蓄積するため使わない。

#### Embed レイアウト
- タイトル: "Timer" (固定。直下に余白行は入れない)
...
ボタン: work/break は無し (表示専用)。
**finalBreak / countdown は Components V2 (Container) で組む** (US-続行)。
- 理由: 最終休憩で「続ける場合:」を画像の下・続行ボタンの上に**太字**で出すため
  (Embed footer は太字不可)。finalBreak→countdown は同一メッセージ edit で遷移するため、
  V2→Embed の edit ができない都合上 countdown も V2 に揃える。
- Container 構成 (上から): TextDisplay `**Timer**` → MediaGallery (円形画像) →
  TextDisplay `-# 設定サマリ` (subtext) → (finalBreak のみ) TextDisplay `**続ける場合:**` →
  (finalBreak のみ) ActionRow「続行」ボタン。accentColor にフェーズ色 (左バー相当)。
- 作成時は flags に `IsComponentsV2 | SuppressNotifications`。
- work↔break (Embed) と finalBreak/countdown (V2) の切替は repost、finalBreak↔countdown は
  V2↔V2 の edit なので整合する。孤児掃除 (purgeOwnEmbeds) は Embed だけでなく
  コンポーネント付きメッセージ (V2 カード) も対象にする。
...
追跡し明示的に削除する。
- 「お疲れさまでした」: 終了演出で投稿し、**投稿から15秒後**に削除 → その直後に新スタート
  Embed を投稿する (お疲れさまを15秒見せてからスタート Embed に切り替える。
  FAREWELL_DELETE_DELAY_MS = 15_000)。「削除→スタート投稿」のフォローアップは setTimeout で
  予約し終了演出はブロックしない。新セッション開始 (onTimerStart) や `/pomo stop` (onIdle)
  が15秒以内なら予約を解除し各処理側でスタート Embed を投稿する。
  → 終了直後〜15秒の間はスタート Embed が無い状態 (お疲れさまのみ表示)。
...
### 孤児プレーンテキストの本文一致掃除 (purgeOwnTexts)
id 追跡は bot 異常終了・再起動で失われ得る (例: ended 直後15秒以内の再起動で残る
お疲れさま投稿)。そこで本文 (content) 完全一致 + 自分の投稿のみを対象に掃除する
purgeOwnTexts を、以下のアクティブセッションが無い/切り替わる地点で呼ぶ:
- 起動時 (setupVoiceFeature) — 再起動で残った孤児を回収
- onTimerStart (新セッション開始・歓迎投稿の前) — 前回 ended のお疲れさま等を一掃
- onIdle (`/pomo stop`・空VC idle 復帰)
本文一致のため、人間が同じ文言を投稿しても author.id が異なり消さない。best-effort。
新セッション開始・idle 復帰時は予約済みの「お疲れさま15秒削除」タイマーも解除する。
