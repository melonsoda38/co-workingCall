...
追跡し明示的に削除する。
- 「お疲れさまでした」: 終了演出で投稿し、**投稿から30秒後**に削除 → その直後に新スタート
  Embed を投稿する (お疲れさまを30秒見せてからスタート Embed に切り替える。
  FAREWELL_DELETE_DELAY_MS = 30_000)。「削除→スタート投稿」のフォローアップは setTimeout で
  予約し終了演出はブロックしない。新セッション開始 (onTimerStart) や `/pomo stop` (onIdle)
  が30秒以内なら予約を解除し各処理側でスタート Embed を投稿する。
  → 終了直後〜30秒の間はスタート Embed が無い状態 (お疲れさまのみ表示)。
...
id 追跡は bot 異常終了・再起動で失われ得る (例: ended 直後30秒以内の再起動で残る
...
新セッション開始・idle 復帰時は予約済みの「お疲れさま30秒削除」タイマーも解除する。
