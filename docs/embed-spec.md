...
更新タイミングは「次の5の倍数秒境界 (残り秒の1の位が0/5)」に吸着させる:
post の API 往復遅延 (数百ms〜2s) を吸収し、表示が "59→54→49" のように
半端な値で並ぶのを防ぐ。具体的には初回 update を setTimeout で境界まで
待機させ、以降は厳密に 5,000ms 間隔の setInterval で発火する。
これにより post の遅延ぶんは初回境界到達までの待ち時間として一度だけ
吸収され、以降ドリフトしない。
