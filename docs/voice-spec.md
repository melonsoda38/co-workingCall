...
[人間ゼロ → 1人以上 になった瞬間]
  bot自動入室 + 入室メッセージ送信
  30秒カウントダウンタイマーがあればキャンセル

[人間1人以上 → ゼロ になった瞬間]
  30秒カウントダウンタイマー開始
   ├ 30秒以内に人間入室 → カウントダウンキャンセル (botは残留)
   └ 30秒経過 → bot退出
...
```
1. 30秒カウントダウンタイマー起動
   - setTimeout で 30秒後に bot退出処理をスケジュール
...
通常の退出フロー (人間ゼロ → ゼロ→30秒待ち → bot退出) ではなく、
