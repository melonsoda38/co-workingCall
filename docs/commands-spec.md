...
- 表示位置: 最終休憩のタイマーカードは **Components V2** (Container) で組み、
  上から「タイトル → 円形タイマー画像 → 設定サマリ(subtext) → **太字**「続ける場合:」→
  続行ボタン」の順に並べる。つまり「続ける場合:」は **Timer 画像の下・続行ボタンの上**に
  太字 (Container 内の TextDisplay `**続ける場合:**`) で表示する。
  「続ける場合:」はテキスト、ボタンは「続行」ラベルのみ。
  (Discord の Embed footer は太字にできないため、画像の下に太字を置くには V2 化が必要。
   finalBreak→countdown は同一メッセージ edit で遷移するため countdown も V2 にする。)
...
- 効果 (詳細は ending-spec.md §続行):
  ...
  - 継続ループは開始時の作業/休憩時間で「作業→休憩→…」を無限に繰り返す
    (セット数による終了はしない)。継続中の画像はセット数の代わりに、
    累計の実施セット数 (元セッションのセット数 + 継続サイクル数) を「N回目」で表示。
