...
- VC 内蔵テキスト欄のスタート Embed から ▶開始 ボタンでタイマー開始 (`/pomo` と同じ許可ロール保持者のみ・VC 参加が前提)
- スタート Embed の 設定 ボタンで作業/休憩/セット数/最終休憩を変更・`config.json` へ永続化 (各項目 1〜999 分、既定 50/10/2/15 分。変更も許可ロール保持者のみ)
